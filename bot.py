...
class MarketAnalyzer:
    ...
    def analyze_single_frame(self, symbol, interval):
        ...
        bb_bandwidth = ((bb_upper - bb_lower) / df['EMA_20'].iloc[-1]) * 100
        ...
        # --- TÍNH TOÁN CHIẾN LƯỢC CHO ĐƠN KHUNG TẠI ĐÂY ---
        trade_plan = ""
        # Nếu rsi trung tính và xu hướng đi ngang -> Đánh Sideway Biên Độ
        if abs(trend_score) == 1 and (40 <= rsi <= 60):
            trade_plan = (
            ...
            trade_plan = (
                ...
                f" ├ Entry (Hồi nhẹ): `{self.factory.format_price(symbol, last_close * 0.996)}`\n"
                f" ├ Stop Loss (SL): `{self.factory.format_price(symbol, support * 0.995)}`\n"
                f" └ Take Profit (TP): `{self.factory.format_price(symbol, last_close * 1.015)}` | `{self.factory.format_price(symbol, resistance)}`"
            )
        else:
            trade_plan = (
                "🎯 **Hướng đề xuất: SHORT (BÁN KHỐNG)**\n"
                f" ├ Entry (Hồi xanh): `{self.factory.format_price(symbol, last_close * 1.004)}`\n"
                f" ├ Stop Loss (SL): `{self.factory.format_price(symbol, resistance * 1.005)}`\n"
                f" └ Take Profit (TP): `{self.factory.format_price(symbol, last_close * 0.985)}` | `{self.factory.format_price(symbol, support)}`"
            )
